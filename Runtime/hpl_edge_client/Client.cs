...
namespace HplEdgeClient.Client
{
    public class HplClient
    {
        ...
        {
            string query = @"query findCharacterTraits(  $addresses: [Bytes!]  $trees: [Bytes!]  $includeProof: Boolean) {  characterTrait(    addresses: $addresses    trees: $trees    includeProof: $includeProof  ) {    address    layer    leaf_idx    name    proof {      root      proof      node_index      leaf      maxDepth      leaf_index      tree_id      canopy_depth    }    tree_id    uri  }}";
        ...
        {
            string query = @"query createCreateCharacterModelTransaction(  $config: CharacterConfigInput!  $project: String!  $authority: String!  $attributes: VecMapGeneric  $mintAs: MintAsInput  $cooldown: CharacterCooldownInput  $equipableCriteria: [String!]  $payer: String  $lutAddresses: [String!]  $computeUnitPrice: Int) {  createCreateCharacterModelTransaction(    config: $config    project: $project    authority: $authority    attributes: $attributes    mintAs: $mintAs    cooldown: $cooldown    equipableCriteria: $equipableCriteria    payer: $payer    lutAddresses: $lutAddresses    computeUnitPrice: $computeUnitPrice  ) {    characterModel    tx {      blockhash      lastValidBlockHeight      transaction    }  }}";
        ...
        public async Task<CreateEquipResourceOnCharacterTransactionReturn> CreateEquipResourceOnCharacterTransaction(CreateEquipResourceOnCharacterTransactionParams parameters, string authToken = null)
        {
            string query = @"query createEquipResourceOnCharacterTransaction(  $characterModel: String!  $characterAddress: String!  $resource: String!  $amount: BigInt!  $owner: String!  $payer: String  $lutAddresses: [String!]  $computeUnitPrice: Int) {  createEquipResourceOnCharacterTransaction(    characterModel: $characterModel    characterAddress: $characterAddress    resource: $resource    amount: $amount    owner: $owner    payer: $payer    lutAddresses: $lutAddresses    computeUnitPrice: $computeUnitPrice  ) {    blockhash    lastValidBlockHeight    transaction  }}";
            var response = await graphQLClient.Query<CreateEquipResourceOnCharacterTransactionReturn, CreateEquipResourceOnCharacterTransactionParams>(query, parameters, authToken);
            return response.GetData();
        }

        public async Task<CreateDismountResourceOnCharacterTransactionReturn> CreateDismountResourceOnCharacterTransaction(CreateDismountResourceOnCharacterTransactionParams parameters, string authToken = null)
        {
            string query = @"query createDismountResourceOnCharacterTransaction(  $characterModel: String!  $characterAddress: String!  $resource: String!  $amount: BigInt!  $owner: String!  $payer: String  $lutAddresses: [String!]  $computeUnitPrice: Int) {  createDismountResourceOnCharacterTransaction(    characterModel: $characterModel    characterAddress: $characterAddress    resource: $resource    amount: $amount    owner: $owner    payer: $payer    lutAddresses: $lutAddresses    computeUnitPrice: $computeUnitPrice  ) {    blockhash    lastValidBlockHeight    transaction  }}";
            var response = await graphQLClient.Query<CreateDismountResourceOnCharacterTransactionReturn, CreateDismountResourceOnCharacterTransactionParams>(query, parameters, authToken);
            return response.GetData();
        }
        ...
        {
            string query = @"query findMissions($addresses: [Bytes!], $missionPools: [Pubkey!]) {  mission(addresses: $addresses, missionPools: $missionPools) {    project    address    program_id    discriminator    bump    missionPool    name    minXp    cost {      amount      resource_address    }    requirement {      ... on TimeRequirement {        kind        params {          duration        }      }    }    rewards {      min      max      rewardType {        ... on XpRewardType {          kind        }        ... on ResourceRewardType {          kind          params {            address          }        }      }    }  }}";
